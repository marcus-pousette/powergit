...
- Latest Playwright run (2025-11-05) fails earlier: `live-cli.spec.ts` aborts because `PSGIT_TEST_REMOTE_URL` isn’t set and `live-github-import.spec.ts` dies when `pnpm --filter @pkg/cli exec tsx src/bin.ts login --guest` exits with \"Unknown argument: guest\"; configure the remote URL + adjust CLI args to re-enable the live suite, then revisit the branch-data regression. Continued PowerSync logs show blank source tables—double-check the TanStack collection wiring.
...
Updated to upstream TanStack PowerSync adapter; collections now rely on default schema conversion.
