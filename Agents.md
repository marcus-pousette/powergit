...
- The dev stack now exports `POWERSYNC_DAEMON_ENDPOINT` + `POWERSYNC_DAEMON_TOKEN` alongside the existing PowerSync env vars; source `.env.powersync-stack` (or re-run `pnpm dev:stack -- --print-exports`) before invoking any CLI/daemon flows so the auto-started daemon authenticates correctly.
